...
9. Configure the OpenGrep worker with `OPENGREP_BIN=opengrep` and `OPENGREP_CONFIG=/path/to/rules.yml` to generate SARIF; the worker fails closed if either setting is absent instead of emitting an empty stub result.
10. Optional: start the triage service with Postgres: `mvn -pl triage-service quarkus:dev`, set `DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD`, configure JWT verification (`MP_JWT_VERIFY_PUBLICKEY_LOCATION` or `MP_JWT_VERIFY_PUBLICKEY`), and set `GIT_CLONE_TOKEN` for private repos; the `role` claim must include `security:vuln_assessor:triager`. UI lives at `http://localhost:8095/triage/` and Swagger at `http://localhost:8095/swagger-ui`. If you want PR URLs in responses, set `TRIAGE_PR_URL_TEMPLATE` with `{repo}`, `{base}`, and `{branch}` placeholders.
11. Optional: set `TRIAGE_SERVICE_URL=http://localhost:8095` on the filter worker to persist potential false positives; configure the triage service policy location/cache via `TRIAGE_POLICY_PATH` and `TRIAGE_POLICY_CACHE_MINUTES` (defaults to `.autotriage/policy.cel` on the repo default branch).
...
3. `scan-api` listens on port 8080, the suppression mock on 8090, the triage service on 8095, and Postgres on 5432; `ARTIFACTS_DIR` is shared across workers via a podman volume.
...
2. Deploy Temporal (with TLS certs mounted via Secrets) and apply the manifests under `k8s/` which wire config as env vars; triage service and database manifests are not yet included.
