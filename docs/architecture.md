...
Health endpoints run in each Quarkus service to satisfy the operational requirement, and Dockerfiles live in `docker/` for each runnable module. The filter worker produces a `suppression-report-*.json` alongside `final.sarif`, and a small `suppression-service-mock` module provides a stub `/ingest` endpoint for early integrations. A GitHub Actions workflow enforces formatting + unit tests as the CI gate.
